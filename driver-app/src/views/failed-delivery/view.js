...
export async function mount(rootElement) {
  ...
  if (routeId && expected_order_id) {
    try {
      ...
      const currentOrder = orders?.find(
        (o) => String(o.OrderID) === String(expected_order_id),
      );
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", async () => {
      const selectedReasonEl = rootElement.querySelector(
      ...
      try {
        ...
        const { status } = await api.patch(
          `/api/v1/dispatch/stops/${active_stop_id}/status`,
          payload,
        );
        ...
        alert(
          "Failed to confirm delivery failure. Please check your connection and try again.",
        );
