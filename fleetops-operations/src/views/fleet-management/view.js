...
async function fetchFleetData() {
  try {
    ...
    if (response.ok && response.data?.success) {
      ...
      console.warn(
        "[FleetOps] fetchFleetData: API returned non-success response.",
      );
    }
  } catch (error) {
...
const MECHANICS = [
  ...
  },
  {
    id: "M-03",
    name: "Faisal Heavy",
    specialty: "Heavy Vehicle Specialist",
    jobs: "3 active job(s)",
    available: false,
  },
  {
...
function updatePageStats() {
  ...
  const inMaint = FLEET_DATA.filter(
    (v) => v.status === "In Maintenance",
  ).length;

  const el = (id) => document.getElementById(id);
  if (el("vehicleCount"))
    el("vehicleCount").textContent = `${FLEET_DATA.length} vehicles in fleet`;
  if (el("alertTitle"))
    el("alertTitle").textContent =
      `${damaged} vehicle(s) damaged — needs mechanic assignment`;
...
function initSearch() {
  ...
  input.addEventListener("input", () => {
    ...
    renderTable(
      q
        ? FLEET_DATA.filter((v) => v.plate.toLowerCase().includes(q))
        : FLEET_DATA,
    );
  });
}
  ...
  if (saveBtn) {
    saveBtn.onclick = () => {
      ...
      const marketValue = document
        .getElementById("fieldMarketValue")
        ?.value?.trim();
      ...
      const newVehicle = {
        ...
        marketValue: marketValue
          ? `SAR ${Number(marketValue).toLocaleString()}`
          : "SAR —",
        lastService: today,
        damageReport: null,
        insurance: "N/A",
...
function renderMechanicList() {
  ...
  list.addEventListener("click", (e) => {
    ...
    list
      .querySelectorAll(".mechanic-item")
...
function initAssignMechanicModal() {
  ...
  const open = (vehicleId) => {
    ...
    if (reportText)
      reportText.textContent =
        vehicle.damageReport || "No damage details provided.";
  ...
  if (confirmBtn) {
    confirmBtn.onclick = () => {
      if (!selectedMechanic) {
        alert("Please select a mechanic before assigning.");
        return;
      }
...
function buildFuelHistory(type) {
  ...
  return Array.from(
    { length: 6 },
    () => +(base + (Math.random() * 1.6 - 0.8)).toFixed(1),
  );
}
...
function loadChartJS(callback) {
  ...
  script.onerror = () =>
    console.error("[FleetOps] Failed to load Chart.js from CDN.");
  document.head.appendChild(script);
}
function destroyCharts() {
  if (odoChartInstance) {
    odoChartInstance.destroy();
    odoChartInstance = null;
  }
  ...
  const commonScales = {
    x: {
      ...
      ticks: {
        ...
        font: { size: 11, family: CHART_DEFAULTS.font },
      },
    },
    y: {
      grid: { color: CHART_DEFAULTS.gridColor, drawBorder: false },
...
function handleGlobalClicks(e) {
  const bell = e.target.closest(
    '.notif-bell-btn, button[aria-label="Notifications"], .bell-btn, #bellBtn',
  );
  if (bell) {
    e.preventDefault();
